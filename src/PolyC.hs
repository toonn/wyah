...
data IState = IState
  { tyctx :: PolyCEnv.Env  -- Type environment
  , tmctx :: TermEnv  -- Value environment
  }

initState :: IState
initState = IState PolyCEnv.empty emptyTmenv
...
showOutput arg st =
  case PolyCEnv.lookup "it" (tyctx st) of
...
typeof args = do
  ...
  case PolyCEnv.lookup arg (tyctx st) of
    Just val -> liftIO $ putStrLn $ ppsignature (arg, val)
...
comp n = do
  ...
  PolyCEnv.TypeEnv ctx <- gets tyctx
